...
//   - paste (Ctrl+V) or drop one/more images -> saved to the plugin's own
//     screenshots/ directory (ephemeral, git-ignored)
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Mirrors the host-side limit (lib/index.js): single file ≤ 50MB.
    ...
    const zh = {
      ...
      "dir.hint": "保存目录",
      "dir.change": "更改",
      "dir.set.failed": "设置目录失败",
      "dir.migrated": "已迁移 {n} 个文件到新目录",
    }
    const en = {
      ...
      "dir.change": "Change",
      "dir.set.failed": "Failed to set directory",
      "dir.migrated": "Migrated {n} file(s) to the new directory",
    }
    ...
    function DockStrip({ useSession, t }) {
      ...
      useEffect(() => {
        // Poll only while the tab is visible: the pending store lives in the
        // host process and other tabs can change it, so a slow poll is the
        // cross-tab sync; a hidden tab gains nothing from polling.
        let timer = 0
        const start = () => {
    ...
    function ScreenshotEntry({ wide, t, insertPath, removePath, getPending }) {
      ...
      // 打开 Windows 原生文件夹选择对话框，选中后设为保存目录
      const pickDir = useCallback(async () => {
        ...
        try {
          ...
          if (typeof payload.migrated === "number" && payload.migrated > 0) {
            setError(L("dir.migrated").replace("{n}", String(payload.migrated)))
          }
          void refresh()
        } catch {
          setError(L("dir.set.failed"))
        }
      }, [L, refresh])

      const saveOne = useCallback(async (file) => {
        // Pre-flight size check: refuse oversized files before the browser
        // reads them fully into memory (the server enforces the same limit).
          ...
          try {
            ...
            for (const file of files) {
              try {
                ...
                // keep going with the rest, but surface why files were skipped
                failed.push(error instanceof Error ? error.message : String(error))
              }
            }
            if (failed.length > 0) {
              setError(L("save.failed") + ": " + failed.join("; "))
            }
            if (saved.length > 0) {
              setPasted(true)
      ...
      useEffect(() => {
        ...
        // Keep the panel's pending list in sync, but only while the tab is
        // visible (the store is host-side and other tabs may change it).
        let interval = 0
        const start = () => {
          window.clearInterval(interval)
          interval = window.setInterval(() => void syncPending(), 1500)
        }
        const onVisibility = () => {
          if (document.hidden) {
            window.clearInterval(interval)
      ...
      const panel = h(
        ...
        h(
          "div",
          { style: styles.footer },
          h("span", { style: { flex: 1 } }, `${L("dir.hint")}: ${dir || "…"}`),
          h("button", { type: "button", style: styles.button, onClick: () => void pickDir() }, L("dir.change")),
          files.length > 0
