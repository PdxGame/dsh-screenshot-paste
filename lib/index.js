...
import { copyFileSync, createReadStream, existsSync, mkdirSync, readdirSync, statSync, unlinkSync, writeFileSync } from "node:fs"
import { dirname, extname, join, resolve } from "node:path"
import { fileURLToPath } from "node:url"
import { homedir } from "node:os"
import { spawn } from "node:child_process"

// Plugin's own directory (resolved from this module, so it works in both the
// source checkout and the installed copy): screenshots live under
// <plugin>/screenshots/ — ephemeral paste-buffer files, git-ignored.
// This module lives in <plugin>/lib/, so go up one level to the plugin root.
const PLUGIN_DIR = resolve(dirname(fileURLToPath(import.meta.url)), "..")
const PLUGIN_SCREENSHOT_DIR = join(PLUGIN_DIR, "screenshots")
...
//   3. <plugin>/screenshots (ephemeral, git-ignored, travels with the plugin)
//   4. ~/.dsh/screenshots — automatic fallback when the plugin dir is not
//      writable, so the plugin still works out of the box.
let SCREENSHOT_DIR = ""
...
/**
 * Open the native Windows folder picker (FolderBrowserDialog) via PowerShell
 * and resolve the selected path; resolves "" when cancelled / unavailable.
 * Non-Windows hosts fall back to the current save directory.
 */
function pickDirectoryViaDialog(initial = "") {
  return new Promise((resolvePromise) => {
    ...
    const ps = [
      ...
      `$d.SelectedPath = '${String(initial).replace(/'/g, "''")}'`,
      "$d.Description = '选择截图保存目录'",
    ...
    child.on("close", () => finish(out.trim().split(/\r?\n/).pop() ?? ""))
  })
}
...
/**
 * Move every accepted screenshot from `fromDir` into `toDir` (copy then
 * delete, so a failed copy never loses the source). Collisions get a numeric
 * suffix instead of being overwritten. Returns a map of old absolute path ->
 * new absolute path for every moved file (empty when nothing moved or the
 * directories are the same). Never throws — a best-effort migration that
 * must not block a directory switch.
 */
function migrateScreenshots(fromDir, toDir) {
  const moves = new Map()
  if (fromDir === "" || toDir === "" || resolve(fromDir) === resolve(toDir)) return moves
  let entries = []
  try {
  ...
  for (const name of entries) {
    ...
    try {
      ...
      while (existsSync(target)) {
        target = join(toDir, `${stem}-${suffix}${ext}`)
        suffix += 1
      }
      ...
      // skip unreadable / failed entries, keep going
    }
  }
  return moves
}
...
/**
 * Rewrite every pending reference whose file was moved by a migration, so a
 * chip stays attached to the file's new location (and the "已引用" state keeps
 * matching). In-place; returns how many references were rewritten.
 */
function remapPendingPaths(moves) {
  if (moves.size === 0) return 0
  let rewritten = 0
  for (const [session, paths] of pendingBySession) {
    const next = paths.map((p) => {
      const target = moves.get(p.path)
      if (target === undefined) return p
      rewritten += 1
      return { ...p, path: target, label: p.label }
    })
    pendingBySession.set(session, next)
  }
  return rewritten
}
...
export async function apply(ctx) {
  ...
  const routes = [
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        if (method === "GET") {
          ...
          const raw = pendingBySession.get(session) ?? []
          // Drop references whose file no longer exists (deleted or moved
          // outside a migration), so chips auto-clear instead of dangling.
          ...
          if (paths.length !== raw.length) pendingBySession.set(session, paths)
    ...
    {
      kind: "exact",
      path: `${API_PREFIX}/pick`,
      handler: async (req, res) => {
        if (!isLoopbackRequest(req)) return void writeJson(res, 403, { error: "forbidden: loopback-only" })
        if (!isTrustedOrigin(req)) return void writeJson(res, 403, { error: "forbidden: cross-site request" })
        if ((req.method ?? "GET") !== "POST") return void writeJson(res, 405, { error: `method not allowed: ${req.method}` })
        // Prefer the official directoryPicker service (native OS dialog);
        // fall back to the PowerShell dialog when the service is unavailable.
        let picked = ""
        try {
          const cap = ctx.directoryPicker?.capability?.()
          if (cap?.kind === "native") {
            const chosen = await cap.pick(new AbortController().signal)
            picked = chosen ?? ""
          }
        } catch {
          picked = ""
        }
        if (picked === "") picked = await pickDirectoryViaDialog(SCREENSHOT_DIR)
        if (picked === "") return void writeJson(res, 200, { cancelled: true })
        const dir = resolve(picked)
        try {
          mkdirSync(dir, { recursive: true })
          if (!statSync(dir).isDirectory()) throw new Error("not a directory")
        } catch {
          return void writeJson(res, 400, { error: "cannot use directory" })
        }
        const previous = SCREENSHOT_DIR
        SCREENSHOT_DIR = dir
        // Persist through the settings namespace when available, so the
        // choice survives restarts (settings hot-applies back to us).
        try {
          ctx.settings?.update?.("dsh-screenshot-paste", { dir })
        } catch {
          // settings unavailable — in-memory change still applies
        }
        // Best-effort migration of previously saved screenshots; rewrite any
        // pending references whose files moved so chips stay attached.
        const moves = migrateScreenshots(previous, SCREENSHOT_DIR)
        remapPendingPaths(moves)
        writeJson(res, 200, { dir: SCREENSHOT_DIR, migrated: moves.size })
      },
    },
    {
      kind: "exact",
      ...
      handler: async (req, res) => {
        ...
        if ((req.method ?? "GET") !== "POST") return void writeJson(res, 405, { error: `method not allowed: ${req.method}` })
        const body = await readJsonBody(req)
        if (body === undefined || typeof body.dir !== "string" || body.dir.trim() === "") {
          return void writeJson(res, 400, { error: "missing dir" })
        }
